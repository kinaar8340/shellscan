...
`capsid-t3-ms2`: hexagons painted `dimer` (parabolic), pentagons stay pentamer. Stoichiometry still 180; generator is the dimer (MS2 / \(\mathrm{TD}_t(1,1)\)). Goldberg face resolution, not 90 rhombs meshed. Compare vs CK at F=32: kind and section \(12/32=0.375\) — agree only on pentagons. Four bins, pentagons elliptic, hexagons parabolic instead of hyperbolic. **Model** + **Software fact**. Not a Hypothesis that MS2 “is” this mesh. pT=3 (VP1/VP2/VP3) is a different YAML on the same net, not opened here.
